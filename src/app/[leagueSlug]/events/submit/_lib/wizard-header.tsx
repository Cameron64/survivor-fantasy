  ...
  const goBack = () => {
    if (currentStep === 'review') {
      ...
      if (selectedType === 'TRIBAL_COUNCIL') {
        router.push(`/${leagueSlug}/events/submit/${slug}/extras`)
      } else {
        router.push(`/${leagueSlug}/events/submit/${slug}`)
      }
      return
    }
    ...
    if (currentStep === 'form' && subStep) {
      ...
      if (idx > 0) {
        router.push(`/${leagueSlug}/events/submit/${slug}/${tcStepOrder[idx - 1]}`)
        return
      }
    }

    if (currentStep === 'form') {
      router.push(`/${leagueSlug}/events/submit`)
    } else {
      router.push(`/${leagueSlug}/events`)
    }
  }
