  ...
  const fetchData = async () => {
    try {
      const [eventsRes, gameEventsRes, episodesRes, contestantsRes, tribesRes] = await Promise.all([
        fetch(`/api/${leagueSlug}/events`),
        fetch(`/api/${leagueSlug}/game-events`),
        fetch(`/api/${leagueSlug}/episodes`),
        fetch(`/api/${leagueSlug}/contestants?includeMemberships=true`),
        fetch(`/api/${leagueSlug}/tribes`),
      ])
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-6">
      ...
      <Button
        ...
        onClick={() => router.push(`/${leagueSlug}/events/submit`)}
