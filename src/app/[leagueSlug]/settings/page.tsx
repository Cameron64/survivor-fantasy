...
import { Settings, Share2, Trash2, RefreshCw, Copy, Check, Bug, FlaskConical, Rocket, ChevronDown, ChevronUp, Database, Zap } from 'lucide-react'
...
interface LeagueData {
  id: string
  name: string
  tier: 'FREE' | 'COMMISSIONER' | 'COMMUNITY'
...
export default function SettingsPage() {
  ...
  const isCommissioner = league?.membershipRole === 'COMMISSIONER'
  ...
  const fetchLeague = async () => {
    try {
      const res = await fetch(`/api/${leagueSlug}/league`)
      if (!res.ok) return
      const data = await res.json()
      // Get current user's membership role from the memberships list
      const meRes = await fetch('/api/users/me')
      const me = await meRes.json()
      const membership = data.memberships?.find((m: { userId: string; role: string }) => m.userId === me.id)
      setLeague({
        id: data.id,
        name: data.name,
        tier: data.tier,
  ...
  const upgradeTier = async (tier: 'COMMISSIONER' | 'COMMUNITY') => {
    if (!league) return
    setIsUpgrading(true)
    try {
      const res = await fetch('/api/stripe/create-checkout', {
        ...
        body: JSON.stringify({ leagueId: league.id, tier, leagueSlug }),
      })
      const data = await res.json()
      if (data.url) {
  ...
  const fetchFeatureFlags = async () => {
    try {
      const res = await fetch(`/api/${leagueSlug}/feature-flags`)
      const data = await res.json()
  ...
  const toggleFeature = async (key: string, checked: boolean, setter: (val: boolean) => void) => {
    ...
    try {
      const res = await fetch('/api/admin/feature-flags', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ [key]: checked, leagueSlug }),
      })
      ...
      {league && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {league.tier === 'FREE'
                ? 'Upgrade to unlock more players and features'
                : `Active until ${league.paidUntil ? new Date(league.paidUntil).toLocaleDateString() : '—'}`}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2">
              <Badge
                variant={league.tier === 'FREE' ? 'outline' : 'success'}
              >
                {league.tier}
              </Badge>
              {league.tier !== 'FREE' && league.paidUntil && (
                <span className="text-sm text-muted-foreground">
                  Active until {new Date(league.paidUntil).toLocaleDateString()}
                </span>
              )}
            </div>

            {league.tier === 'FREE' && isCommissioner && (
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-lg border p-4 space-y-3">
                  <div>
                    <p className="font-semibold">Commissioner</p>
                    <p className="text-2xl font-bold mt-1">$49 <span className="text-sm font-normal text-muted-foreground">/ season</span></p>
                  ...
                  <Button
                    className="w-full"
                    onClick={() => upgradeTier('COMMISSIONER')}
                    disabled={isUpgrading}
                  >
                    {isUpgrading ? <RefreshCw className="h-4 w-4 mr-2 animate-spin" /> : <Zap className="h-4 w-4 mr-2" />}
                    Upgrade
                  </Button>
                </div>
                ...
                <div className="rounded-lg border border-primary p-4 space-y-3">
                  <div>
                    ...
                    <p className="text-2xl font-bold mt-1">$99 <span className="text-sm font-normal text-muted-foreground">/ season</span></p>
                    <ul className="text-sm text-muted-foreground mt-2 space-y-1">
            ...
            {league.tier === 'FREE' && !isCommissioner && (
              <p className="text-sm text-muted-foreground">
                Only the league commissioner can upgrade the plan.
              </p>
            )}
          </CardContent>
        </Card>
