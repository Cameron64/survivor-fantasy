/**
 * Callback query handler for Survivor Fantasy Telegram Bot.
 *
 * Handles inline keyboard button presses (ask_user MCP integration + session resume).
 */
...
export async function handleCallback(ctx: Context): Promise<void> {
  ...
  // 3. Load request file
  const requestFile = `${TEMP_DIR}/ask-user-${requestId}.json`;
  let requestData: {
    question: string;
    options: string[];
    status: string;
