/**
 * Configuration for Survivor Fantasy Telegram Bot.
 *
...
process.env.PATH = pathParts.join(PATH_SEP);
...
const DEFAULT_WORKING_DIR =
  "C:/Users/Cam Dowdle/source/repos/personal/survivor-fantasy";
export const WORKING_DIR = process.env.CLAUDE_WORKING_DIR || DEFAULT_WORKING_DIR;
...
// Build safety prompt for survivor-fantasy repo
...
5. PROJECT-SPECIFIC RULES (Survivor Fantasy League):
   - Use pnpm (not npm or yarn) for all package operations
   - Be careful with database operations (pnpm db:push, pnpm db:migrate)
   - Never store scores directly in the database - scores derive from approved Events
   - When running tests, use: pnpm test (unit) or pnpm test:e2e (Playwright)
   - For deployments, the app deploys to Railway via GitHub Actions
...
const thinkingKeywordsStr = process.env.THINKING_KEYWORDS || "think";
const thinkingDeepKeywordsStr =
  process.env.THINKING_DEEP_KEYWORDS || "think hard,ultrathink";
...
export const AUDIT_LOG_PATH =
  process.env.AUDIT_LOG_PATH || `${TMP}/survivor-bot-audit.log`;
...
export const SESSION_FILE = `${TMP}/survivor-bot-session.json`;
export const RESTART_FILE = `${TMP}/survivor-bot-restart.json`;
export const TEMP_DIR = `${TMP}/survivor-bot`;

// Temp paths that are always allowed for bot operations
export const TEMP_PATHS = ["/tmp/", "/private/tmp/", "/var/folders/", TMP + "/"];
