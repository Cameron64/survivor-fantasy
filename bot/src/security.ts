/**
 * Security module for Survivor Fantasy Telegram Bot.
 *
...
export function isPathAllowed(path: string): boolean {
  try {
    // Normalize slashes to forward slashes for consistent comparison on Windows
    ...
    let resolved: string;
    try {
      resolved = toForward(realpathSync(normalized));
    } catch {
      resolved = toForward(resolve(normalized));
    }

    // Always allow temp paths (for bot's own files)
    for (const tempPath of TEMP_PATHS) {
      if (resolved.startsWith(toForward(tempPath))) {
        return true;
      }
    }
    ...
    for (const allowed of ALLOWED_PATHS) {
      const allowedResolved = toForward(resolve(allowed));
      if (
